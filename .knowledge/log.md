...
## [2026-07-24] add | [PF-006 + PF-007 + PS-007 雷暴联动分析经验] | 新增 3 条经验：时区陷阱、高风区雷暴检测失效、联动分析流程 | #b3d7e2f1

- 新增 PF-006：Pandas 时区 tz-naive 与 tz-aware 比较错误（verified）
  - 4 个陷阱：Meteostat tz-naive、ERCOT 多源不统一、numpy datetime64 丢时区、utc=True 误用
  - 核心方案：所有加载函数统一 tz_localize(\"UTC\")
- 新增 PF-007：雷暴检测在高风区绝对阈值失效（verified）
  - 4 个陷阱：德州风速基线过高、骤变阈值需调优、系统级聚合平滑信号、空间错配稀释
  - 核心方案：高风区用\"风速骤变\"逻辑替代\"绝对+静风\"逻辑
- 新增 PS-007：雷暴事件 × 电力市场联动分析流程（verified）
  - 完整 5 步流程：事件检测 → 数据对齐 → 窗口分析 → 统计检验 → 分组分析
  - ERCOT 验证结果：113 事件、22 个电价尖峰、LZ_WEST 捕获 3 个 Hub 遗漏事件
  - 关键发现：HB_WEST vs LZ_WEST 相关性仅 0.59，光伏骤降是主要机制
  - 结算点选择建议：风电区用 LZ_WEST/LZ_NORTH，系统级用 HB
